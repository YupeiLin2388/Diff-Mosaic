...
def save_model_and_result(dt_string, epoch,train_loss, test_loss, best_iou, recall, precision, save_mIoU_dir, save_other_metric_dir):
    ...
    with open(save_other_metric_dir, 'a') as f:
        ...
        for i in range(len(precision)):
            ...
            f.write('   ')
        f.write('\n')
def save_all(mean_IOU, save_dir, save_prefix, train_loss, test_loss, recall, precision, epoch):
        save_mIoU_dir = 'result/' + save_dir + '/' + save_prefix + '_all_IoU_IoU.log'
        save_other_metric_dir = 'result/' + save_dir + '/' + save_prefix + '_all_IoU_other_metric.log'
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        #best_iou = mean_IOU
...
def save_Pred_GT(pred, labels, dataset_name,target_image_path, val_img_ids, num, suffix):

    predsss = np.array((pred > 0).cpu()).astype('int64') * 255
    predsss = np.uint8(predsss)
    labelsss = labels * 255
    labelsss = np.uint8(labelsss.cpu())
    labels = Image.open(os.path.join('./dataset/'+dataset_name+'/images/',val_img_ids[num]+suffix))
    #img = Image.fromarray(predsss.reshape(labels.size[0], labels.size[1]))
    img = Image.fromarray(predsss.reshape(256, 256))
    #img = img.resize((labels.size[0], labels.size[1]))
    img.save(os.path.join(target_image_path,(val_img_ids[num]) +suffix))
